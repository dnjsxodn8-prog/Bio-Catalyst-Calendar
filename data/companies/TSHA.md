...
## 플랫폼
- **self-complementary AAV9 (scAAV9)** — 척수강내 전달
- **miRARE (miRNA-Responsive Auto-Regulatory Element)** — MECP2 과발현 방지를 위한 세포별 자동 발현 조절 원소. 과발현 독성을 내재적 miRNA 피드백으로 억제.
- CNS 단일유전자 질환 특화

## 적응증
- Rett syndrome (MECP2 기능소실 돌연변이, 여성) — TSHA-102 임상 진행 중

## 파트너
- **UT Southwestern Medical Center** — TSHA-102 원천기술 라이선스(Steven Gray 교수 연구)
- **국제 Rett 증후군 재단(IRSF)** — 자연경과 데이터 제공, Part B 시험 디자인 지원
- 주요 임상기관: Boston Children's Hospital, UCSD 등

## 매출 구조
제품 매출 없음. 수익은 임시 collaboration 수익/보조금. 향후 TSHA-102 승인 시 희귀질환 유전자치료제 고가(one-time) 수익 모델 예상.

## 자체 판매
미정 (상업화 전). 승인 후 자체 판매 또는 파트너사 통한 유통 결정 예정.
...
## 기존 치료제
- **Daybue (trofinetide, Acadia Pharmaceuticals)** — 2023년 3월 FDA 최초 승인. IGF-1 N말단 삼펩타이드 합성 유사체. 증상 개선(소통, 미세운동, 호흡)이지만 질환 수정 효과 없음. 연간 약 $375,000. 2025 Q1 매출 $84.6M.
- **DAYBUE STIX (분말 제형)** — 2026년 승인, Q2 광범위 출시 예정.
- 대증 요법(항뇌전증제, 물리치료, 작업치료)만 존재.
...
## MOA
- 타겟: MECP2 (methyl-CpG binding protein 2) — X염색체 Xq28 위치
- 기전: scAAV9 벡터가 척수강내 투여 후 CSF를 통해 CNS 세포에 분산 → 기능적 miniMECP2 유전자 발현 → MeCP2 단백질 보충 → 신경세포 성숙·시냅스 기능·후성유전 조절 회복. miRARE 원소가 세포별 내인성 miRNA 피드백으로 MeCP2 발현량을 자동 조절 → 과발현 독성 방지.
- 병태생리: MECP2 기능소실 → MeCP2 단백질 결여 → 전사 조절 이상 → 신경세포 성숙 지연, 시냅스 가소성 저하, 글리아 기능 장애 → Rett 표현형(발달퇴행, 뇌전증, 자율신경 이상). 유전자 보충으로 이 근본 결함을 직접 교정.
...
- ASGCT 2026 (2026-04-27 발표): 신규 전임상 데이터 발표 예정.
